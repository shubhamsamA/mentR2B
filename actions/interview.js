...
export async function generateQuiz(formData) {
  ...
  // Use formData if passed, else fallback to last submitted InterviewForm
  let form;
  if (formData) {
    form = formData;
  } else {
  ...
  const prompt = `
    Generate ${form.numQuestions} ${form.type} interview questions for a ${
    form.industry
  } professional${form.skills.length ? ` with expertise in ${form.skills.join(", ")}` : ""}.
